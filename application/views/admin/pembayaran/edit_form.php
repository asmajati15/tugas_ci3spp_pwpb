			...
			<main class="py-6 bg-surface-secondary">
				<div class="container-fluid">
					<div class="card mb-7">
						...
						<div class="card-body">
                        <form action="<?php echo site_url('pembayaran/edit/'. $pembayaran->id_pembayaran) ?>" method="post" enctype="multipart/form-data">
						<!-- Note: atribut action dikosongkan, artinya action-nya akan diproses 
							...
							<div class="form-group mb-4">
								...
								<select class="form-select <?php echo form_error('id_petugas') ? 'is-invalid':'' ?>"
									...
									<?php foreach ($petugass as $petugas): ?>
										<option <?php if($pembayaran->id_petugas ==$petugas->id_petugas){ echo 'selected="selected"'; } ?> value="<?php echo $pembayaran->id_petugas ?>"><?php echo $petugas->nama_petugas ?></option>
									<?php endforeach; ?>
								</select>
							...
							<div class="form-group mb-4">
								...
								<select class="form-select <?php echo form_error('id_siswa') ? 'is-invalid':'' ?>"
									...
									<?php foreach ($siswas as $siswa): ?>
										<option <?php if($pembayaran->id_siswa ==$siswa->id_siswa){ echo 'selected="selected"'; } ?> value="<?php echo $pembayaran->id_siswa ?>"><?php echo $siswa->nama ?></option>
									<?php endforeach; ?>
								</select>
								<div class="invalid-feedback">
							...
							<div class="form-group mb-4">
								...
								<input class="form-control <?php echo form_error('tanggal_bayar') ? 'is-invalid':'' ?>"
								 type="date" name="tanggal_bayar" value="<?php echo $pembayaran->tanggal_bayar ?>" />
								<div class="invalid-feedback">
							...
							<div class="form-group mb-4">
								...
								type="text" name="jumlah_bayar" min="0" placeholder="Example: Rp200.000" value="<?php echo $pembayaran->jumlah_bayar ?>" />
								<div class="invalid-feedback">
							...
							<div class="form-group mb-4">
								...
								<select class="form-select <?php echo form_error('id_spp') ? 'is-invalid':'' ?>"
									...
									<?php foreach ($spps as $spp): ?>
										<option <?php if($pembayaran->id_spp ==$spp->id_spp){ echo 'selected="selected"'; } ?> value="<?php echo $pembayaran->id_spp ?>"><?php echo $spp->tahun_ajaran ?>(<?php echo $spp->nominal ?>)</option>
									<?php endforeach; ?>
								</select>
								<div class="invalid-feedback">
